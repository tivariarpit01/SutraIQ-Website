 ...
 */
export async function POST(request: Request) { // <--- THIS LINE IS CRUCIAL
  try {
    ...
    if (phoneNumber && typeof phoneNumber !== 'string') {
      return NextResponse.json({ message: 'Phone number must be a string.' }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { message: 'Your message has been received and saved successfully!', id: newContactMessage._id },
      { status: 200 }
    );

  } catch (error) {
    console.error('Error processing contact form submission or saving to DB:', error);
    return NextResponse.json(
